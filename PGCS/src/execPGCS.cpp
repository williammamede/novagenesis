...
void generateApplicationLifecycleReport(string Path, string _Time)
{
	...
	cout << "(Creating the json file that will store the system state)" << endl;
	ifstream reportFile;
	reportFile.open(Path + "ApplicationLifecycleReport.json");
	string reportContent;
	if (reportFile.is_open())
	{
		cout << "(The file already exists. Deleting it)" << endl;
		std::remove((Path + "ApplicationLifecycleReport.json").c_str());
	}
	reportFile.close();

	reportContent = "{\n \"serviceStates\": []}";
	string pgcsServiceState = "{\n \"serviceName\": \"PGCS\",\n \"serviceState\": \"starting\",\n \"time\": \"" + _Time + "\"\n}";
	string nrncsServiceState = "{\n \"serviceName\": \"NRNCS\",\n \"serviceState\": \"waiting\",\n \"time\": \"" + _Time + "\"\n}";
	string app1ServiceState = "{\n \"serviceName\": \"ContentApp1\",\n \"serviceState\": \"waiting\",\n \"time\": \"" + _Time + "\"\n}";
	string app2ServiceState = "{\n \"serviceName\": \"ContentApp2\",\n \"serviceState\": \"waiting\",\n \"time\": \"" + _Time + "\"\n}";

	ofstream reportFile2;
	reportFile2.open(Path + "ApplicationLifecycleReport.json");
	if (reportFile2.is_open())
	{
		reportFile2 << reportContent;
		reportFile2.close();
	}
	else
	{
	...
	// Open the json file and add the service states
	cout << "(Opening the json file and adding the service states)" << endl;
	ifstream reportFile3;
	reportFile3.open(Path + "ApplicationLifecycleReport.json");
	string reportContent2;
	if (reportFile3.is_open())
	{
		string line;
		while (getline(reportFile3, line))
		{
			reportContent2 += line;
		}
		reportFile3.close();
	...
	// Add the service states to the json file
	cout << "(Adding the service states to the json file)" << endl;
	ofstream reportFile4;
	reportFile4.open(Path + "ApplicationLifecycleReport.json");
	if (reportFile4.is_open())
	{
		reportFile4 << reportContent2.substr(0, reportContent2.length() - 2);
		reportFile4 << "\n" + pgcsServiceState + ",\n" + nrncsServiceState + ",\n" + app1ServiceState + ",\n" + app2ServiceState + "\n]}";
		reportFile4.close();
	}
	else
	...
	cout << "(The application lifecycle report was generated)" << endl;
}
...
int main(int argc, char *argv[])
{
	...
	if (argc >= 5)
	{
		...
		generateApplicationLifecycleReport(Path,getTime());
	...
	if (Problem == true)
	{
		...
		cout
			...
			<< endl;
		cout << "(Usage: ./PGCS Path Port Role -l to run locally without peers.)" << endl
			 << endl;
	}

...
#ifdef DEBUG
							  PB->S << Offset << "(The following key matched = " << Key.at (t) << ")" << endl;

							  // Open debug file
							  PCore->Debug.OpenOutputFile ();

							  PCore->Debug
								  << "A category 18 binding was received. A previous created subscription with index "
								  << i << " matched the key = " << PS->Key << ". Status of the subscription is "
								  << PS->Status << endl;
